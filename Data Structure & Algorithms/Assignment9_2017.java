...
public class Assignment9_2017Solution {
...
class BinarySearchTree<E extends Comparable<E>> implements Iterable<E>{
    ...
    public List<E> get(Predicate<E> pr){
        List<E> lst = new ArrayList<>();
        getMatches(root,lst,pr);
        return lst;
    }
    private void getMatches(BNode<E> rt, List<E> ls, Predicate<E> pr){
        if(rt != null){
            getMatches(rt.left(), ls, pr);
            if(pr.test(rt.data()))ls.add(rt.data());
            getMatches(rt.right(),ls, pr);
        }
    }
